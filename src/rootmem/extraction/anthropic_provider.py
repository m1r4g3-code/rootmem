...
_SYSTEM_PROMPT = (
    ...
    "object_literal instead. Use a small, consistent, singular, capitalized "
    "vocabulary for entity types (Person, Organization, Location, Date, "
    "etc.) — the same reasoning as predicates applies: the same entity type "
    "string is later used to look entities back up, so picking a different "
    "casing or synonym per mention (e.g. 'person' vs 'Person', or "
    "'Company' vs 'Organization') is a comparison error, not a style "
    "choice, even though lookups are also normalized case-insensitively as "
    "a second line of defense. Set confidence lower (below 0.5) only when "
