...
3. **Candidate contradiction**: same `(subject_entity_id, predicate)` but a *different* object. The new relation's own belief is computed exactly like case 1 (a fresh prior plus this one event) — it does not touch the old relation's belief, since this evidence isn't about the old relation's claim. **New evidence wins by default, including a tie**: the old relation is superseded (`valid_to` set, `supersedes`/`superseded_by` linked, exactly as Phase 1's mechanics already do) *unless* `old_confidence > new_confidence + bayesian_supersede_margin` (default margin `0.05`), in which case both relations are marked `metadata.contested = true` and remain active instead, unchanged from Phase 1's escape hatch. Framing the comparison this way — contest only when the *old* belief is meaningfully stronger, not require the *new* belief to clear an absolute bar — is deliberate: two single-shot, equally-confident, equally-reliable facts are a genuine evidentiary tie, and a tie should resolve the same way Phase 1's most-recent-wins baseline already did for the ordinary case, not newly default to `contested` for every routine fact update. Only the *decision function* changed from Phase 1, not the soft-supersede plumbing. This is the concrete mechanism behind the exit criterion's requirement (d): a fresh, single, uncorroborated relation offers a new contradiction no real resistance (it still supersedes, matching Phase 1's behavior); three corroborating restatements raise the old relation's confidence enough that the same contradiction can no longer clear the now-higher bar.
